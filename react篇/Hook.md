...
useState和类组件的setState一样，如果新的 state 需要通过使用先前的 state 计算得出，那么可以将函数传递给 `setState`。该函数将接收先前的 state，并返回一个更新后的值。
...
- 每次调用自定义Hook，它都会获取独立的state，即便是一个组件内多次调用
- Hook本身就是函数，可以在参数来传递信息。当我们传入不同的参数时，会返回不同的结果。
